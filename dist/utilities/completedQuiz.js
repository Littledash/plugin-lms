 ...
 */ const findCompletedQuiz = (courseProgress, quiz)=>{
    const quizId = typeof quiz === 'object' ? quiz.id : quiz;
    return courseProgress.completedQuizzes.find((cq)=>typeof cq.quiz === 'object' ? cq.quiz.id === quizId : cq.quiz === quizId) || null;
};
/**
 * Checks if a quiz is completed for a specific course
 ...
 */ export const isQuizCompleted = (progress, courseId, quiz)=>{
 ...
 */ const isScorePassing = (score, minimumScore)=>{
 ...
 */ export const isQuizPassed = (progress, courseId, quiz)=>{
    const courseProgress = progress.find((p)=>(typeof p.course === 'object' ? p.course.id : p.course) === courseId);
    if (!courseProgress) {
        return false;
    }
 ...
 */ export const completedQuiz = (progress, courseId, quiz)=>{
    const courseProgress = progress.find((p)=>(typeof p.course === 'object' ? p.course.id : p.course) === courseId);
    if (!courseProgress) {
        return {
            isCompleted: false,
    ...
    const passed = typeof quiz === 'object' && quiz.minimumScore !== undefined ? isScorePassing(completedQuizEntry.score, quiz.minimumScore) : null;
    return {
        isCompleted: true,
        passed,
...
//# sourceMappingURL=completedQuiz.js.map