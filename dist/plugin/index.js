 ...
 */ export const lmsPlugin = (pluginConfig) => (incomingConfig) => {
  ...
  if (existingStudentsCollection) {
    ...
    const existingRolesField = findFieldByNameAndType(
      existingStudentsCollection?.fields || [],
      'roles',
      'select',
    )
    ...
    const existingEnrolledCoursesField = findFieldByNameAndType(
      existingStudentsCollection?.fields || [],
      'enrolledCourses',
      'relationship',
    )
    ...
    const existingCompletedCoursesField = findFieldByNameAndType(
      existingStudentsCollection?.fields || [],
      'completedCourses',
      'relationship',
    )
    ...
    const existingCoursesProgressField = findFieldByNameAndType(
      existingStudentsCollection?.fields || [],
      'coursesProgress',
      'array',
    )
    ...
    const exisitingCertificatesField = findFieldByNameAndType(
      existingStudentsCollection?.fields || [],
      'certificates',
      'relationship',
    )
  ...
  const currenciesConfig = pluginConfig.currencies ?? {
    defaultCurrency: 'AUD',
  ...
  if (
    incomingConfig.collections?.find((col) => col.slug === 'categories') &&
    ...
    if (existingCategories) {
      existingCategories.fields = deepMerge(
